  ...
  async init() {
    ...
    globalPastScript.text = `
        ...
        function findTextarea(event) {
            ...
            return el ? el.closest('textarea') : null;
        }
        ...
        function showFloatingButton(event) {
            ...
            if(!activeTextarea || activeTextarea.id == "weilin_pastmask_post_textarea" || activeTextarea.id == "weilin_pastmask_neg_textarea"){return}
            ...
            if (!isFloatingButtonVisible) {
                ...
                // 防止点击时文本框失焦
                positiveButton.onmousedown = function(e) { e.preventDefault(); };
                negativeButton.onmousedown = function(e) { e.preventDefault(); };

                function updateComfyUITextarea(textarea, newText) {
                    if (!textarea) return;

                    // 记录覆盖前文本，用于寻找底层节点
                    const oldText = textarea.value;

                    // 1. 强行更新前端 DOM 层 (视觉表现层)
                    ...
                    // 2. ★终极解决核心★: 彻底绕过前端 Vue 的拦截，直接将数据写入底层的 LiteGraph 引擎中！
                    if (window.app && window.app.graph) {
                        let matchedWidgets = [];
                        
                        // 遍历所有节点，找出所有当前内容匹配的 Widget 组件
                        for (const node of window.app.graph._nodes) {
                            if (!node.widgets) continue;
                            for (const w of node.widgets) {
                                if (typeof w.value === 'string' && w.value === oldText) {
                                    matchedWidgets.push({ widget: w, node: node });
                                }
                            }
                        }

                        if (matchedWidgets.length > 0) {
                            const selectedNodes = (window.app.canvas && window.app.canvas.selected_nodes) ? window.app.canvas.selected_nodes : {};
                            let updated = false;

                            // 优先更新用户当前鼠标选中的那个节点
                            for (const match of matchedWidgets) {
                                if (matchedWidgets.length > 1 && !selectedNodes[match.node.id]) continue;
                                match.widget.value = newText;  // 【核心写入】
                                if (typeof match.widget.callback === 'function') {
                                    try { match.widget.callback(newText, window.app.canvas, match.node, null, null); } catch(e){}
                                }
                                updated = true;
                            }
                            
                            // 兜底方案：如果没有选中的节点，直接更新第一个匹配的
                            if (!updated && matchedWidgets.length > 0) {
                                matchedWidgets[0].widget.value = newText;
                                if (typeof matchedWidgets[0].widget.callback === 'function') {
                                    try { matchedWidgets[0].widget.callback(newText, window.app.canvas, matchedWidgets[0].node, null, null); } catch(e){}
                                }
                            }
                        }

                        // 通知核心引擎发生了修改，这不仅会让任务正确读取新数据，还会瞬间自动激发出星号 (*)！
                ...
                positiveButton.onclick = function () {
                    ...
                    updateComfyUITextarea(activeTextarea, thisInputGreatElement.value);
                };

                negativeButton.onclick = function () {
                    let thisInputNegElement = document.getElementById("weilin_global_neg_prompt_input");
                    if (!activeTextarea) return;
                    updateComfyUITextarea(activeTextarea, thisInputNegElement.value);
                };
        ...
        window.addEventListener('click', function (event) {
            ...
            if(thisInputGreatElement.value.length > 0 || thisInputNegElement.value.length > 0){
                const getConfig = localStorage.getItem("weilin_prompt_global_past_setting")
                if(getConfig == 1){
                    // 修复潜在的 activeTextarea 为空引发的报错
                    if (!floatingButton.contains(event.target) && (!activeTextarea || !activeTextarea.contains(event.target))) {
                        hideFloatingButton();
                    }
                }
            }
